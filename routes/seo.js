...
const DEFAULT_DESC =
  'Partage tes recettes, découvre celles de la communauté, gagne des points en cuisinant.';
...
router.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    const r = await db.get(
      'SELECT title, description, photo, is_hidden, status FROM recipes WHERE id=$1',
      [id]
    );
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const rows = await db.all(
      "SELECT id, created_at FROM recipes WHERE is_hidden=0 AND status='published' ORDER BY created_at DESC LIMIT 5000"
    );
    const urls = [
      `<url><loc>${base}/</loc><changefreq>daily</changefreq></url>`,
      ...rows.map(
        (r) =>
          `<url><loc>${base}/recette/${r.id}</loc><lastmod>${r.created_at.slice(0, 10)}</lastmod></url>`
      ),
    ].join('\n');
    res
