// Tests d'intégration : démarre l'app Express sur un port éphémère avec une base SQLite
// en mémoire (isolée du fichier cuistot.db réel), et tape dedans avec fetch.
...
test('inscription (mdp < 10 caractères refusé), doublon refusé, mauvais mot de passe refusé, connexion ok', async () => {
  ...
  const tooShort = await s.call('/register', { method: 'POST', body: { username: 'alice_test', email, password: 'short1' } });
  assert.equal(tooShort.status, 400);
  assert.equal(tooShort.data.code, 'VALIDATION_ERROR');
  assert.ok(tooShort.data.fields.password);

  const reg = await s.call('/register', { method: 'POST', body: { username: 'alice_test', email, password: 'secret12345' } });
  assert.equal(reg.status, 200);
  assert.equal(reg.data.user.username, 'alice_test');

  const dup = await s.call('/register', { method: 'POST', body: { username: 'alice_test2', email, password: 'secret12345' } });
  assert.equal(dup.status, 409);
  assert.equal(dup.data.code, 'CONFLICT');

  const badLogin = await s.call('/login', { method: 'POST', body: { email, password: 'wrongpassword' } });
  assert.equal(badLogin.status, 401);

  const login = await s.call('/login', { method: 'POST', body: { email, password: 'secret12345' } });
...
test('création de recette : le champ image est filtré (corrige une ancienne faille XSS), rate limit like', async () => {
  ...
  await s.call('/register', { method: 'POST', body: { username: 'bob_test', email, password: 'secret12345' } });
...
test('validation stricte : titre vide et description trop longue refusés', async () => {
  ...
  await s.call('/register', { method: 'POST', body: { username: 'gina_test', email, password: 'secret12345' } });
  ...
  const tooLong = await s.call('/recipes', { method: 'POST', body: { title: 'Titre ok', description: 'x'.repeat(2001), tags: [] } });
...
test('un champ numérique vidé (temps de préparation) retombe sur la valeur par défaut au lieu d\'échouer', async () => {
  ...
  await s.call('/register', { method: 'POST', body: { username: 'henri_test', email, password: 'secret12345' } });

  const created = await s.call('/recipes', { method: 'POST', body: { title: 'Soupe', prep_minutes: '', tags: [] } });
  assert.equal(created.status, 200);
...
test('mot de passe oublié -> réinitialisation -> connexion avec le nouveau mot de passe', async () => {
  ...
  await s.call('/register', { method: 'POST', body: { username: 'carol_test', email, password: 'oldpassword1' } });
  await s.call('/logout', { method: 'POST' });

  const forgot = await s.call('/forgot-password', { method: 'POST', body: { email } });
  assert.equal(forgot.status, 200);
  assert.ok(forgot.data.devToken, 'devToken doit être renvoyé hors production, pour pouvoir tester le flux');

  const reset = await s.call('/reset-password', { method: 'POST', body: { token: forgot.data.devToken, password: 'newpassword1' } });
  assert.equal(reset.status, 200);

  const oldLogin = await s.call('/login', { method: 'POST', body: { email, password: 'oldpassword1' } });
  assert.equal(oldLogin.status, 401);

  const newLogin = await s.call('/login', { method: 'POST', body: { email, password: 'newpassword1' } });
  assert.equal(newLogin.status, 200);

  // le jeton est à usage unique : une seconde utilisation doit échouer
  const reuse = await s.call('/reset-password', { method: 'POST', body: { token: forgot.data.devToken, password: 'again12345' } });
  assert.equal(reuse.status, 400);
});

test('email inconnu sur /forgot-password : même réponse générique (pas d\'énumération de comptes)', async () => {
  const s = makeSession();
  const res = await s.call('/forgot-password', { method: 'POST', body: { email: `inconnu_${Date.now()}@test.fr` } });
  assert.equal(res.status, 200);
...
test('avatar : icône libre acceptée, clé inconnue retombe sur "user", couleur invalide sur "cream"', async () => {
  ...
  await s.call('/register', { method: 'POST', body: { username: 'dave_test', email, password: 'secret12345' } });

  const ok = await s.call('/me', { method: 'PATCH', body: { avatar: 'leaf' } });
  assert.equal(ok.data.user.avatar, 'leaf');

  const bad = await s.call('/me', { method: 'PATCH', body: { avatar: '<script>alert(1)</script>' } });
...
test('avatar à débloquer : refusé tant que le badge requis n\'est pas obtenu', async () => {
  ...
  await s.call('/register', { method: 'POST', body: { username: 'frank_test', email, password: 'secret12345' } });
...
test('pagination du fil de recettes', async () => {
  ...
  const reg = await s.call('/register', { method: 'POST', body: { username: 'eve_test', email, password: 'secret12345' } });
...
test('un utilisateur ne peut ni modifier ni supprimer la recette d\'un autre', async () => {
  ...
  await owner.call('/register', { method: 'POST', body: { username: 'owner_test', email: ownerEmail, password: 'secret12345' } });
  await intruder.call('/register', { method: 'POST', body: { username: 'intruder_test', email: intruderEmail, password: 'secret12345' } });

  const created = await owner.call('/recipes', { method: 'POST', body: { title: 'Recette privée', tags: [] } });
  const id = created.data.recipe.id;

  const putAttempt = await intruder.call(`/recipes/${id}`, { method: 'PUT', body: { title: 'Piraté', tags: [] } });
...
test('un commentaire trop court est publié mais ne rapporte pas de points (Lot 9)', async () => {
  ...
  await author.call('/register', { method: 'POST', body: { username: 'author_test2', email: `author2_${Date.now()}@test.fr`, password: 'secret12345' } });
  const commenterReg = await commenter.call('/register', { method: 'POST', body: { username: 'commenter_test', email: `commenter_${Date.now()}@test.fr`, password: 'secret12345' } });
  const pointsBefore = commenterReg.data.user.points;

  const recipe = await author.call('/recipes', { method: 'POST', body: { title: 'Recette à commenter', tags: [] } });
  const id = recipe.data.recipe.id;

  const shortComment = await commenter.call(`/recipes/${id}/comments`, { method: 'POST', body: { body: 'ok' } });
...
test('signalement : créé par un utilisateur, visible et traitable par l\'admin, refusé pour un non-admin', async () => {
  ...
  await reporter.call('/register', { method: 'POST', body: { username: 'reporter_test', email: `reporter_${Date.now()}@test.fr`, password: 'secret12345' } });
  await other.call('/register', { method: 'POST', body: { username: 'other_test', email: `other_${Date.now()}@test.fr`, password: 'secret12345' } });

  const recipe = await other.call('/recipes', { method: 'POST', body: { title: 'Recette à signaler', tags: [] } });
  const recipeId = recipe.data.recipe.id;

  const report = await reporter.call('/reports', { method: 'POST', body: { target_type: 'recipe', target_id: recipeId, reason: 'Contenu inapproprié' } });
  ...
  const login = await admin.call('/login', { method: 'POST', body: { email: 'demo@cuistot.fr', password: 'demo123' } });
  assert.equal(login.status, 200);
  ...
  const resolve = await admin.call(`/admin/reports/${found.id}/resolve`, { method: 'POST', body: { action: 'hide' } });
...
test('blocage : le contenu de l\'utilisateur bloqué disparaît du fil du bloqueur', async () => {
  ...
  await blocker.call('/register', { method: 'POST', body: { username: 'blocker_test', email: `blocker_${Date.now()}@test.fr`, password: 'secret12345' } });
  const blockedReg = await blocked.call('/register', { method: 'POST', body: { username: 'blocked_test', email: `blocked_${Date.now()}@test.fr`, password: 'secret12345' } });
  const blockedId = blockedReg.data.user.id;

  await blocked.call('/recipes', { method: 'POST', body: { title: 'Recette du bloqué', tags: [] } });
...
test('suppression de compte : mot de passe vérifié, compte et contenu supprimés (cascade)', async () => {
  ...
  await s.call('/register', { method: 'POST', body: { username: 'delete_test', email, password: 'secret12345' } });
  await s.call('/recipes', { method: 'POST', body: { title: 'Recette à effacer', tags: [] } });

  const badPassword = await s.call('/me', { method: 'DELETE', body: { password: 'wrongpassword' } });
  assert.equal(badPassword.status, 401);

  const del = await s.call('/me', { method: 'DELETE', body: { password: 'secret12345' } });
  assert.equal(del.status, 200);

  const loginAfter = await s.call('/login', { method: 'POST', body: { email, password: 'secret12345' } });
...
test('export de données : ne contient que le profil et les recettes de l\'utilisateur authentifié', async () => {
  const s = makeSession();
  await s.call('/register', { method: 'POST', body: { username: 'export_test', email: `export_${Date.now()}@test.fr`, password: 'secret12345' } });
